"""
🌾 Kaggriculture Master SOTA Agent — Paced Ranch & Orchard Compounding Engine (V110)
...
def agent(obs):
    ...
    if hour == 1:
        if day == 0:
            ...
            # Strawberries scale to 28 (Days 4 to 12)
            if day >= 4 and day <= 12:
                ...
                if desired_straw > 0 and spendable_money >= 100:
                    buy_s = min(desired_straw, int(spendable_money // 100), 6)
                    if buy_s > 0:
            ...
            if day >= 6 and day <= 22:
                max_cows_target = 12 if unlocked_quads >= 3 else (4 if unlocked_quads >= 2 else 2)
