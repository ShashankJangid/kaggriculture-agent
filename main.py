...
def agent(obs):
    ...
    # 1. Market Selling: Sell everything from shed every single turn
    for item, qty in list(shed.items()):
        if qty > 0:
            market_orders.append(["SELL", item, qty])

    # 2. Strategic Land Expansion with Liquidity Buffers
    unlocked_quads = len(farm.get("unlocked_quadrants", ["NW"]))
    if unlocked_quads < 4:
        next_cost = LAND_PRICES[unlocked_quads - 1]
        # Day 0: Buy NE quadrant ($1000) immediately leaving $2000 buffer
        # Later: Buy next quadrant only if money >= cost + 600
        min_buffer = 400 if day == 0 else 600
        if money >= next_cost + min_buffer and day <= 20:
            market_orders.append(["BUY_LAND"])
            money -= next_cost
            unlocked_quads += 1

    # 3. Dynamic Liquidity-Aware Hiring
    ...
    if day >= 28:
        ...
        # Scale with money and unlocked quadrants
        if money < 300:
            target_hires = 2
        elif money < 800:
            target_hires = 4
            ...
            target_hires = 8 if unlocked_quads >= 3 else 6

    if hour == 0 and hires_today < target_hires and money >= 80:
    ...
    if day <= 18:
        # High profit melon + fast carrot cycle
        max_melons = 16 if unlocked_quads >= 2 else 8
        desired_melon = max(0, max_melons - melon_tiles - seeds.get("MELON", 0))
        if desired_melon > 0 and money >= 80:
            buy_melon = min(desired_melon, int(money // 80), 8)
            if buy_melon > 0:
                market_orders.append(["BUY_SEED", "MELON", buy_melon])
                money -= buy_melon * 80

        target_carrot = 20 + (pet_cafes * 10)
        if seeds.get("CARROT", 0) < target_carrot and money >= 20:
            buy_carrot = min(target_carrot - seeds.get("CARROT", 0), int(money // 20), 10)
            if buy_carrot > 0:
                market_orders.append(["BUY_SEED", "CARROT", buy_carrot])
                money -= buy_carrot * 20

        if seeds.get("WHEAT", 0) < 10 and money >= 10:
            buy_wheat = min(10 - seeds.get("WHEAT", 0), int(money // 10), 10)
            if buy_wheat > 0:
        ...
        # Endgame Sprint: Carrots only
        if seeds.get("CARROT", 0) < 20 and money >= 20:
            buy_carrot = min(20 - seeds.get("CARROT", 0), int(money // 20), 10)
            if buy_carrot > 0:
                market_orders.append(["BUY_SEED", "CARROT", buy_carrot])
                money -= buy_carrot * 20
    ...
    # Priority 1: WATER unwatered plants (prevents weeds / maximizes yield!)
    # Priority 2: HARVEST ripe plants
    # Priority 3: DIG weeds
    # Priority 4: PLANT empty tiles
    for y in range(board_size):
        for x in range(board_size):
            tile = farm["tiles"][y][x]
            if tile == "LOCKED":
    ...
    for idx, unit_pos in enumerate(all_units):
        ...
        # Shed navigation if carrying >= 4 items
