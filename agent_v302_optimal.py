"""
Agent V302 — V203 + Globally Optimal Greedy Task Assignment
Author: Shashank Jangid

Analysis of V203 ($40,943 avg): Workers spend 49% of all turns just MOVING.
With 9 workers × 720 turns = 6480 total turns, ~3175 are travel.
If we can reduce travel by 10%, that's 317 extra productive actions = ~$1,500-$2,500.

ROOT CAUSE of excessive travel:
  Current dispatch: Assigns worker 0 to nearest task first, then worker 1 to
  nearest REMAINING task, etc. Worker 0 (near shed) takes the shed-adjacent
  task; worker 1 finds next nearest; etc. Workers can end up taking tasks far
  from their current position if closer tasks are "stolen" by higher-priority workers.

FIX in V302 — Optimal Greedy Matching:
  Build all (distance, worker_idx, task_idx) pairs for all unassigned workers
  and tasks. Sort by distance. Greedily assign the globally closest pair first,
  then continue. This ensures the globally MINIMUM travel distance assignment.
  
  Example: 3 workers at positions A, B, C. 3 tasks at X, Y, Z.
  Old: Worker A takes nearest (X), worker B takes nearest remaining (Y),
       worker C gets Z even if Z is close to A.
  New: Assign the closest (worker, task) pair first globally → optimal packing.
...
def agent(obs):
    ...
    # 1. Shed liquidation (UNCHANGED)
    for item, qty in list(shed.items()):
        if qty > 0:
            market_orders.append(["SELL", item, qty])

    # 2. Hiring (UNCHANGED from V203)
    ...
    # 5. Cournot opponent + town shops (UNCHANGED)
    ...
    # ── 8. IMPROVED DISPATCH: Globally Optimal Greedy Task Assignment ─────────
    # V203 dispatched worker 0 first (greedy in unit order).
    # V302: build all (dist, worker, task) pairs, sort, assign globally closest first.
    unit_actions     = [None] * num_units
    assigned_tiles   = set()
    local_seeds      = dict(seeds)
    unassigned_units = set(range(num_units))

    # Pass 1: Standing-tile actions & shed drops (UNCHANGED)
    for u_idx in list(unassigned_units):
        ...
        if carrying > 0 and is_shed_adj:
            ...
            unassigned_units.discard(u_idx)
            continue
    ...
    # Build all valid (dist, u_idx, task_idx) triples
    all_pairs = []
    for u_idx in unassigned_units:
        ...
        for t_idx, task in enumerate(ordered_tasks):
            ...
            if tpos in assigned_tiles:
                continue
            # Weight: distance + priority offset (lower task index = higher priority)
            # Priority tasks (watering, harvesting) get distance-1 bonus to win ties
            priority_offset = t_idx * 0.1  # tiny offset so watering wins over planting at same distance
            d = manhattan_dist((ux, uy), tpos) + priority_offset
            all_pairs.append((d, u_idx, t_idx))

    all_pairs.sort()  # sort by effective distance (closest globally-optimal pair first)

    assigned_workers = set()
    for _, u_idx, t_idx in all_pairs:
        if u_idx in assigned_workers:
            continue
        task = ordered_tasks[t_idx]
        tpos = task["pos"]
        if tpos in assigned_tiles:
            continue

        # Assign this (worker, task) pair
        assigned_tiles.add(tpos)
        assigned_workers.add(u_idx)

        ux, uy = all_units[u_idx]
        mv = get_best_move((ux, uy), tpos)
        if mv:
            unit_actions[u_idx] = [mv]
        else:
            ttype = task["type"]
    ...
    # Workers with no task → rally to shed
    for u_idx in unassigned_units:
        if u_idx in assigned_workers:
            continue
        ux, uy = all_units[u_idx]
