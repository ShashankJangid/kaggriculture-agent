...
def agent(obs):
    ...
    # 1. Immediate Market Liquidation
    ...
    if hour < 20:
        if day <= 17:
            ...
            if seeds.get("WHEAT", 0) < 5 and spendable_money >= 10:
                buy_w = min(5 - seeds.get("WHEAT", 0), int(spendable_money // 10), 5)
                if buy_w > 0:
            ...
            if seeds.get("WHEAT", 0) < total_unlocked_tiles and spendable_money >= 10:
                buy_w = min(total_unlocked_tiles - seeds.get("WHEAT", 0), int(spendable_money // 10), 25)
                if buy_w > 0:
                    market_orders.append(["BUY_SEED", "WHEAT", buy_w])
                    spendable_money -= buy_w * 10

    # 6. Build Priority Queues (V8 EXACT Logic - WATER strictly before HARVEST)
    tasks_watering = []
    tasks_harvesting = []
    tasks_digging = []
    tasks_planting = []
