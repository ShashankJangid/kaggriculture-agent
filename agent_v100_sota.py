"""
🌾 Kaggriculture Master SOTA Agent — 140k Replicated Compounding Engine
Author: Shashank Jangid
Exact Reverse-Engineered Playbook of Rank #1 (Ryo Hasegawa, 140,348 coins)
"""
import math
...
def agent(obs):
    ...
    # 1. Market Sales: Keep reserve of Wheat for animal feed, sell everything else
    wheat_in_shed = shed.get("WHEAT", 0)
    for item, qty in list(shed.items()):
        if qty > 0:
            if item == "WHEAT":
                surplus = max(0, qty - (18 if day >= 11 else 6))
                if surplus > 0:
    ...
    hiring_reserve = 300 if day >= 10 else 100
    ...
    if hour < 20:
        if day == 0:
            if total_sheep < 2 and spendable_money >= 300:
                buy_sh = min(2 - total_sheep, int(spendable_money // 300))
                if buy_sh > 0:
                    market_orders.append(["BUY_ANIMAL", "SHEEP", buy_sh])
                    spendable_money -= buy_sh * 300
                    total_sheep += buy_sh
            if total_cows < 2 and spendable_money >= 400:
                buy_cw = min(2 - total_cows, int(spendable_money // 400))
                if buy_cw > 0:
                    market_orders.append(["BUY_ANIMAL", "COW", buy_cw])
                    spendable_money -= buy_cw * 400
                    total_cows += buy_cw
        ...
        elif day >= 4 and day <= 12:
            # Strawberries scale to 30
            target_strawberries = 30 if unlocked_quads >= 2 else 12
            ...
            if desired_strawberries > 0 and spendable_money >= 100:
                buy_s = min(desired_strawberries, int(spendable_money // 100), 8)
                if buy_s > 0:
                    market_orders.append(["BUY_SEED", "STRAWBERRY", buy_s])
                    spendable_money -= buy_s * 100

        elif day >= 6 and day <= 24:
            # Cow herd scales up to 16
            max_cows_target = 16 if unlocked_quads >= 3 else (6 if unlocked_quads >= 2 else 2)
            if total_cows < max_cows_target and spendable_money >= 400 + 600:
                buy_c = min(max_cows_target - total_cows, int((spendable_money - 600) // 400), 2)
                if buy_c > 0:
                    market_orders.append(["BUY_ANIMAL", "COW", buy_c])
                    spendable_money -= buy_c * 400
                    total_cows += buy_c

        # Wheat maintenance for animal feed and cash
        if day >= 1 and day <= 27:
            target_wheat = 28 if unlocked_quads >= 3 else (15 if unlocked_quads >= 2 else 6)
        ...
        # Feed buffer in shed
        if (total_cows + total_sheep > 0) and wheat_in_shed < (10 if day >= 11 else 4) and spendable_money >= 30:
            buy_feed = min(10, int(spendable_money // 25), 12 - wheat_in_shed)
            if buy_feed > 0:
