"""
🌾 Autonomous Industrial Farm Agent v500 — Apex Market Arbitrage & Compounding Engine
Author: Shashank Jangid

Key Breakthroughs:
1. Dynamic Market Price Arbitrage: Real-time price tracker exploiting town shop demand spikes.
2. 75-Tile Optimal Footprint: 3 quadrants (NW, NE, SW) ensuring 100% hydration & zero transport bottleneck.
3. Surgical Dual-Wave Melons: 20 melons in Wave 1 + 20 melons in Wave 2 (max yield $30k each wave).
4. Town Shop Exploitation:
   - PET_CAFE (Carrot demand): Scales carrot production dynamically.
   - BAKERY / FARMERS_MARKET: Scales wheat demand dynamically.
5. Labor Flow & Zero-Waste Logistics:
   - Unit-first spatial dispatch preventing task collisions.
   - Day 25-29 Sustained workforce (6 workers on D25-27, 4 workers on D28-29) ensuring 0 stranded crops.
"""
import math
...
def agent(obs):
    ...
    # 1. Real-time Market Liquidation (Sell all shed inventory immediately)
    ...
    # 4. Analyze Farm Tiles & Crop Counts
    ...
    # 5. Market Insights & Dynamic Town Demands
    ...
    if hour < 20:
        if day <= 18:
            ...
            # Dynamic Carrot Target (Adjusted by Pet Cafe + Market Price)
            opp_carrots = opp_crops.get("CARROT", 0)
            price_bonus = 0.2 if carrot_price > 40 else 0.0
            carrot_mult = 1.0 + (pet_cafes * 0.4) + price_bonus - (0.1 if opp_carrots > 25 and pet_cafes == 0 else 0.0)
            ...
            desired_wheat = max(0, 32 - seeds.get("WHEAT", 0))
