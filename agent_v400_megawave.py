"""
🌾 Autonomous Industrial Farm Agent v400 — Mega-Wave 100-Tile Compounding Engine
Author: Shashank Jangid

Breakthrough Architecture:
1. Wave 1 (Days 0-12): 20 Melons + Carrots/Wheat on 50 tiles -> generates $30,000+ cash.
2. Day 13 Mega Expansion: Full 100-tile unlock (Quads 3 + 4) using Wave 1 capital.
3. Wave 2 (Days 13-25): 50 Mega-Melons planted Day 13 -> matures Day 25 (yield 300 melons = +$75,000).
4. Wave 3 (Days 25-29): 70+ Wheat rapid-cycle sprint on all 100 tiles -> +$15,000.
5. Scaled Workforce: 6 workers early -> 12 workers mid-game -> 8 workers endgame.
"""
import math
...
def agent(obs):
    ...
    # 1. Immediate Shed Liquidation
    for item, qty in list(shed.items()):
        if qty > 0:
            market_orders.append(["SELL", item, qty])

    # 2. Workforce Dynamic Hiring
    ...
    # 3. Dynamic Land Expansion (Full 100-Tile Scaler)
    ...
    # 4. Count Farm Tile States
    melon_tiles = carrot_tiles = wheat_tiles = 0
    ...
    if hour < 20:
        # Phase 1: Days 0 - 12 (Wave 1 Setup)
        if day <= 12:
        ...
        # Phase 2: Days 13 - 16 (Wave 2 MEGA MELONS)
        elif day <= 16:
            # Plant up to 45 melons for Wave 2 (matures Day 25)
            max_melons = 45 if unlocked_quads >= 3 else 25
            desired_melons = max(0, max_melons - melon_tiles - seeds.get("MELON", 0))
            if desired_melons > 0 and spendable_money >= 80:
                buy_m = min(desired_melons, int(spendable_money // 80), 15)
                if buy_m > 0:
                    market_orders.append(["BUY_SEED", "MELON", buy_m])
                    spendable_money -= buy_m * 80
        ...
        # Phase 3: Days 17 - 24 (Midgame Bridge)
        elif day <= 24:
            target_carrots = 40 + (pet_cafes * 15)
            desired_carrots = max(0, target_carrots - carrot_tiles - seeds.get("CARROT", 0))
            if desired_carrots > 0 and spendable_money >= 20:
            ...
            if seeds.get("WHEAT", 0) < 20 and spendable_money >= 10:
                buy_w = min(20 - seeds.get("WHEAT", 0), int(spendable_money // 10), 10)
                if buy_w > 0:
                    market_orders.append(["BUY_SEED", "WHEAT", buy_w])
                    spendable_money -= buy_w * 10

        # Phase 4: Days 25 - 27 (Endgame Full-Board Wheat Sprint)
        elif day <= 27:
            desired_wheat = max(0, 50 - seeds.get("WHEAT", 0))
