"""
🌾 Kaggriculture Master SOTA Agent — Dedicated Sector Architecture (V130)
...
def agent(obs):
    ...
    if hour == 1:
        if day == 0:
            ...
            # Wheat Maintenance (Top Priority - Must maintain 6-28 Wheat fields!)
            ...
            if tile is None:
                if (x, y) not in [t["pos"] for t in tasks_build_pasture]:
                    if day < 28 and hour < 20:
                        # Determine intended sector for this tile
                        if wheat_tiles < (28 if unlocked_quads >= 3 else (14 if unlocked_quads >= 2 else 6)) and seeds.get("WHEAT", 0) > 0:
                            tasks_planting_wheat.append({"type": "PLANT_WHEAT", "pos": (x, y)})
                        elif strawberry_tiles < 28 and seeds.get("STRAWBERRY", 0) > 0 and day <= 12:
                            tasks_planting_straw.append({"type": "PLANT_STRAW", "pos": (x, y)})
                        elif melon_tiles < 11 and seeds.get("MELON", 0) > 0 and day <= 8:
                ...
                if kind == "WEED":
                    ...
                    if not watered:
                        if crop in ["STRAWBERRY", "MELON", "WHEAT"] or tile.get("consecutive_unwatered", 0) >= 1:
    ...
    for task in ordered_tasks:
        ...
        if best_u is not None:
            ...
            elif ttype == "PLANT_STRAW":
                mv = get_best_move((ux, uy), tpos, board_size)
                if mv:
                    unit_actions[best_u] = [mv]
                else:
                    if local_seeds.get("STRAWBERRY", 0) > 0:
