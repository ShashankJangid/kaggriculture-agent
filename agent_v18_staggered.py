...
def agent(obs):
    ...
    if hour < 20:
        if day <= 17:
            # DYNAMIC MELON UNCAP!
            # If we harvested our first Melons, we have a massive cash injection.
            if spendable_money > 10000:
                max_melons = 100
            elif unlocked_quads >= 3:
                max_melons = 40
            elif unlocked_quads >= 2:
            ...
            if desired_melons > 0 and spendable_money >= 80:
                # TRICK: Staggered seed buying (cap at 8) ensures plants mature over 12+ hours, preventing shed overflow!
    ...
    for u_idx in list(unassigned_units):
        ...
        # TRICK 2: PREVENT SHED OVERFLOW
        # If dropping this would overflow the shed, WAIT until market sells it next hour.
        if sum(u_inv.values()) > 0 and is_shed_adj:
            # We can't access total shed sum accurately if multiple workers drop, but the engine handles it.
            # Wait! We CAN just use a PASS if shedding is likely to overflow!
            # But the engine handles overflow by keeping the rest in the backpack.
            # SO all we need to do is keep issuing DROP! The engine will accept what fits, and keep the rest.
