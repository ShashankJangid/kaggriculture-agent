...
def agent(obs):
    ...
    if hour < 19:
        if day <= 17:
            # Phase 1: Pure Melon/Carrot Scaling
            ...
            if tile is None:
                ...
                if kind == "WEED":
                    ...
                    if crop_data.get("ongoing", False):
                        ...
                        # Mathematical Master Harvest Logic!
                        max_y = crop_data.get("max_yield", 4)
                        max_d = crop_data.get("max_yield_day", 4)
                        # Harvest if max units reached, OR age limit reached, OR end of game
                        if age >= crop_data.get("first_yield_day", 2):
                            if yield_units >= max_y or age >= max_d or day >= 29:
