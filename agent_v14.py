...
def agent(obs):
    ...
    if hour < 19:
        local_m = seeds.get("MELON", 0)
        local_c = seeds.get("CARROT", 0)
        local_w = seeds.get("WHEAT", 0)

        # Cap seeds at ~50 to avoid draining all money pointlessly
        target_m_cap = max(0, 50 - local_m)
        target_c_cap = max(0, 50 - local_c)
        target_w_cap = max(0, 50 - local_w)

        if day <= 17:
            melon_alloc = spendable_money * 0.70
            carrot_alloc = spendable_money * 0.30

            if spendable_money >= 80 and target_m_cap > 0:
                buy_m = min(target_m_cap, min(60, int(melon_alloc // 80)))
                if buy_m > 0:
                    market_orders.append(["BUY_SEED", "MELON", buy_m])
                    spendable_money -= buy_m * 80

            if spendable_money >= 20 and target_c_cap > 0:
                buy_c = min(target_c_cap, min(50, int(spendable_money // 20)))
                if buy_c > 0:
            ...
            if spendable_money >= 20 and target_c_cap > 0:
                buy_c = min(target_c_cap, min(60, int(spendable_money // 20)))
                if buy_c > 0:
                    market_orders.append(["BUY_SEED", "CARROT", buy_c])
                    spendable_money -= buy_c * 20

        elif day <= 27:
            if spendable_money >= 10 and target_w_cap > 0:
                buy_w = min(target_w_cap, min(40, int(spendable_money // 10)))
                if buy_w > 0:
    ...
    for u_idx in list(unassigned_units):
        ...
        # Reduce drop-off threshold to 2 to ensure we get cash faster
        if sum(u_inv.values()) >= 2:
