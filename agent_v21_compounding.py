"""
🌾 Kaggriculture Autonomous Master Agent v21 - Dynamic Forward Compounding Engine
Author: Shashank Jangid
"""
import math
from collections import defaultdict

# ── Crop Constants & Economics ───────────────────────────────────────────────
CROPS = {
...
def agent(obs):
    ...
    # 1. 100% Instant Market Liquidation (Sell Shed Inventory Immediately)
    for item, qty in list(shed.items()):
        if qty > 0:
            market_orders.append(["SELL", item, qty])

    # 2. Optimal Labor Arbitrage (Fibonacci Cost Scaling)
    ...
    # 3. High-Velocity Compounding Land Acquisition (NE -> SW -> SE)
    ...
    # 5. Dynamic Compounding Seed Arbitrage
    ...
    if hour < 20:
        if day <= 18:
            # Melons: Maximum compounding returns for Day 0-18
            max_melons = 22 if unlocked_quads >= 2 else 11
            ...
            # Carrots for fast 3-day continuous turnover
            target_carrots = 28 + (pet_cafes * 15)
            ...
            # Final Sprint (Days 25-27): Carrots mature in exactly 3 days (by Day 30)
            desired_carrots = max(0, 35 - seeds.get("CARROT", 0))
            if desired_carrots > 0 and spendable_money >= 20:
    ...
    # Pass 1: Zero-Travel Immediate Execution (at current unit position)
