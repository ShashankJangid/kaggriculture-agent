"""
Autonomous Industrial Farm Agent v1300 — Sovereign Apex
Author: Shashank Jangid

V1300 = V1000 + ONE surgical fix: the definitive animal purchase guard.

WHAT CHANGED from V1000:
  The single root-cause bug: animals bought before a built pasture was available,
  stranding 1-4 animals in the shed for 15+ days ($400-$1600 dead capital, plus
  missing daily yields).

  OLD guard (V1000): shed.get("COW",0)==0 and shed.get("SHEEP",0)==0
    Problem: this only stops buying if LAST purchase is still in shed.
    On seed 42: pasture slots fill to 7, V1000 buys 8th animal → it walks to shed
    as no pasture is free → stalls there forever.

  NEW guard (V1300): shed_animals == 0 AND n_anim < n_past
    n_anim = animals currently PLACED on tile (counted from tile scan)
    n_past = pastures currently BUILT on tile (counted from tile scan)
    => Buy ONLY when a built pasture is literally vacant right now AND no animal
       is already waiting in shed. Zero false positives.

Everything else (strawberry timing Day 9, wheat cap, land unlock, hiring)
is IDENTICAL to V1000 to preserve its working seeds.
"""
...
def agent(obs):
    ...
    for item, qty in list(shed.items()):
        ...
        elif qty > 12 and item == "WHEAT":   # trimmed from V1000's 18
    ...
    n_anim  = len(animal_pos)   # animals PLACED on tiles right now
    n_past  = len(pasture_pos)  # pastures BUILT on tiles right now
    ...
    # ── 5. PURCHASING ─────────────────────────────────────────────────────────
    if hour < 20:
        shed_anim = shed.get("COW",0) + shed.get("SHEEP",0)

        # Day 0: buy 2 Cows + 2 Sheep immediately
        if day == 0 and hour == 0:
            if spnd >= 1800:
        ...
        # ★ THE DEFINITIVE GUARD: shed empty AND a built pasture is actually vacant
        elif day <= 12 and shed_anim == 0 and n_anim < n_past:
            if n_cows < 6 and spnd >= 800:
                mkt.append(["BUY_ANIMAL","COW",1]);   spnd -= 400
            elif n_sheep < 5 and spnd >= 900:
        ...
        if day <= 5:
            ...
            # Strawberries from Day 6 like V1000 (budget-gated naturally)
    ...
    for px,py in PASTURE_SLOTS:
        ...
        if t is None and n_past < max_past:
    ...
    ordered = t_anim + t_harv + t_water + t_dig + t_plant + t_build
    ...
    for u in list(todo):
        ...
        if utile not in (None,"LOCKED") and isinstance(utile,dict):
            ...
            if (ux,uy) in PASTURE_SLOTS and n_past<max_past:
