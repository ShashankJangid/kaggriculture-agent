"""
Autonomous Industrial Farm Agent v1400 — Sovereign Apex
Author: Shashank Jangid

V1400 = V1000 + ONE precision fix: late-game harvest-first priority.

ROOT CAUSE DISCOVERY (trace analysis):
- Seeds 42 and 777 have IDENTICAL farm states through Day 15, even Day 25 ($53,785 each).
- Divergence is in the LAST 4 days: V1000 earns $9.5k while V1100b/V1300 earn $30k on Seed 42.
- The task-ordering change (harvest before water) is what drives the +$20k on Seed 42:
  workers collect all ripe strawberry yields FIRST, then water. In V1000 (water-first),
  some yields are missed because workers hit carry-cap after watering and return to shed.
- BUT harvest-first hurts Seeds 777/999 in MID-GAME: strawberries are harvested but some
  miss their watering window, delaying the NEXT harvest cycle.

FIX: hybrid ordering:
  - Days 0-22: water first then harvest (V1000 behavior — preserves working seeds)
  - Days 23-29: harvest first then water (collects all ripe yields in final sweep)

Other V1000 changes kept identical:
  - Same purchasing logic (speculative pipeline buying, works on 8/10 seeds)
  - Wheat shed cap lowered 18→12 (minor capital release, no downside)
"""
...
def agent(obs):
    ...
    # ── 5. PURCHASING (identical to V1000) ────────────────────────────────────
    if hour < 20:
        ...
        # V1000 original guard: shed empty → buy (speculative pipeline)
        elif day <= 12 and shed.get("COW",0)==0 and shed.get("SHEEP",0)==0:
            if num_cows < 6 and spnd >= 800:
                mkt.append(["BUY_ANIMAL","COW",1]);   spnd -= 400
            elif num_sheep < 5 and spnd >= 900:
    ...
    # ★ KEY FIX: water-first early/mid (V1000 behavior), harvest-first late game
    if day >= 23:
