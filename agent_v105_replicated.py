"""
🌾 Kaggriculture Master SOTA Agent — Replicated Rank #1 Engine (V105)
...
def agent(obs):
    ...
    feed_reserve = 18 if day >= 11 else 4
    ...
    # 3. Progressive Land Expansion (75% Cap = 3 Quads max)
    hiring_reserve = 250 if day >= 10 else 50
    spendable_money = max(0, money - hiring_reserve)

    if unlocked_quads < 3 and day >= 5 and day <= 18:
        next_cost = LAND_PRICES[unlocked_quads - 1]
        buffer = 200 if day <= 7 else 400
    ...
    # 5. Market Purchases (Only execute on Hour 1)
    if hour == 1:
        if day == 0:
            if total_sheep < 2:
            ...
            # Strawberries scale to 30 (Days 5 to 12)
            if day >= 5 and day <= 12:
                target_straw = 30 if unlocked_quads >= 2 else 12
            ...
            # Cow herd scaling (Days 6 to 24)
            if day >= 6 and day <= 24:
                max_cows_target = 16 if unlocked_quads >= 3 else (6 if unlocked_quads >= 2 else 2)
                if total_cows < max_cows_target and spendable_money >= 400 + 500:
                    buy_c = min(max_cows_target - total_cows, int((spendable_money - 500) // 400), 2)
                    if buy_c > 0:
