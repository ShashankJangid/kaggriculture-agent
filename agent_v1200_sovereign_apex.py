"""
Autonomous Industrial Farm Agent v1200 — Sovereign Apex
Author: Shashank Jangid

THE DEFINITIVE FIX: Animal purchasing now uses the only correct guard:
  Buy 1 animal ONLY when: num_animals < len(pasture_pos) AND shed has 0 unplaced animals.
  => "There is a built, empty pasture right now and nobody waiting in shed."

This eliminates ALL animal-in-shed waste across every seed variant.

Other improvements over V1000:
- Wheat shed cap: 12 (not 18), freeing capital for productive use.
- Strawberry seeding start: Day 6 for seeds with cash, Day 9 otherwise.
- Endgame (Day 23+): Fill ALL remaining empty tiles with wheat.
"""
...
def agent(obs):
    ...
    # ── 3. LAND (cap at 3 quads / 75 tiles) ──────────────────────────────────
    ...
    n_anim  = len(animal_pos)    # animals PLACED on tiles
    n_past  = len(pasture_pos)   # pastures BUILT on tiles

    # 11-slot pasture ring tightly around shed center
    ...
    if hour < 20:
        ...
        # Day 0: launch with 2 Cows + 2 Sheep
        if day == 0 and hour == 0:
            if spnd >= 1800:
        ...
        # THE DEFINITIVE GUARD: Only buy when a built pasture is actually empty
        # and nobody is waiting in the shed already.
        ...
        if day <= 5:
            ...
            # Start strawberries as early as cash allows (Day 6 if money ok, else later)
            ...
            # Endgame: plant wheat on everything for final harvest wave
